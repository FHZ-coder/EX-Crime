...
def ssl_compute(normalized_embedded_s1, normalized_embedded_s2):
            ...
            ssl_mi = torch.log(torch.exp(pos_score/args.ssl_temp) / torch.exp(all_score/args.ssl_temp).sum(dim=1, keepdim=False)).mean()
            return ssl_mi

def info_nce_loss(query, key, temperature=1.2):
